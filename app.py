...
# Step 3: Preprocessing (grayscale + blur)
...
# Step 6: Skeletonize (convert to boolean then skeletonize)
binary = dilated > 0
...
# Step 7: Vector tracing with Potrace
bmp = potrace.Bitmap(skeleton.astype(np.uint8))
path = bmp.trace()

# Helper: Build SVG from traced paths
def paths_to_svg(path_obj, width, height):
    svg_parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}" xmlns:xlink="http://www.w3.org/1999/xlink">'
    ]
